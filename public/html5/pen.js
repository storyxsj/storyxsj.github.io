...
function Pen(canvasId, isTest) {
	...
	this.Circle = function(name){
		...
		this.isSelected = function(xn,yn,hasElementSelected){
			...
			var key = parseInt((this.x-xn)*(this.x-xn)) + parseInt((this.y-yn)*(this.y-yn)); 
			if(this.r*this.r - key > 0){
				this.select = true;
			}else{
				this.select = false;
			}
		...
		this.draw = function(isRedraw) {
			...
			if(name != null){
				...
				//图像下方
				var y_text = parseInt(this.y)+parseInt(this.r)+parseInt(defaultTextHeith);
			...
			if(this.select){
				//this.context.fillStyle = defaultFillStyle;
				pen.fillCircle(parseInt(this.x), parseInt(this.y), 
						parseInt(this.r)+5);
				
				var mes = "name:"+circle.name+PenDefaultMesSplit+"x:"+circle.x
				pen.hoverTip(circle.x-circle.r,circle.y,mes);
			}

			if(!isRedraw){
				pen.cashNodeList.push(circle);
			}
